<?php
 $db = db_connect();
 $query = $db->query("SELECT * FROM barang_masuk JOIN barang ON barang.id = barang_masuk.id_barang WHERE tgl BETWEEN '$dari' AND '$sampai' AND status = 'Pembelian'");
 //you get result as an array in here but fetch your result however you feel to
 $result = $query->getResultArray();

 $query1 = $db->query("SELECT SUM(harga) as total FROM barang_masuk JOIN barang ON barang.id = barang_masuk.id_barang WHERE tgl BETWEEN '$dari' AND '$sampai' AND status = 'Pembelian'");
 //you get result as an array in here but fetch your result however you feel to
 $row = $query1->getRow();
?>
