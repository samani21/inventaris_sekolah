...
class PelaksanaanPembelajaranModel extends Model
{
    ...
    protected $allowedFields    = ['id', 'id_guru', 'id_mapel', 'materi', 'tanggal', 'id_metode', 'evaluasi', 'foto/video', 'id_user_verifikasi'];
    ...
    public function cetakDataBeetwen($dari, $sampai)
    {
        return $this->join('guru', 'guru.id=pelaksanaan_pembelajaran.id_guru')
            ...
            ->join('mapel', 'mapel.id=pelaksanaan_pembelajaran.id_mapel')
            ->join('metode', 'metode.id=pelaksanaan_pembelajaran.id_metode')
            ->select('guru.nama,guru.nip,pelaksanaan_pembelajaran.*,users.level,mapel.nama_mapel,metode.metode')
            ->where("pelaksanaan_pembelajaran.tanggal BETWEEN '$dari' AND '$sampai'")
